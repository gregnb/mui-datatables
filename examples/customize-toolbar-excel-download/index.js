...
const Example = () => {
  ...
  const handleXLSXDownload = () => {
    if (!!tableState) {
      ...
      const filteredRowIndexArray = tableState.dData.map(row => row.dataIndex);

      const data = tableState.data.filter(row => filteredRowIndexArray.includes(row.index));
      ...
      const data = sheetHeader.concat(filteredData);
      console.log(data);
      /*
