...
|**`isRowSelectable`**|function||Enable/disable selection on certain rows with custom function. Returns true if not provided. `function(dataIndex, selectedRows) => bool`. `selectedRows` contains information about selected rows in following format: `{ data // array of data objects, lookup // map of selected ids }`
